...
export default function ButtonSettings() {
    ...
    const inheritFromThemeRedux = useSelector((state) => state.requeststock.buttonInheritFromTheme);
    ...
    const [requestStockButtonText, setRequestStockButtonText] = useState("Request Stock");
    ...
    function setRequestStockButtonSettings(settings) {
        if(settings != null && settings != undefined && Object.keys(settings).length !== 0)
        {
            ...
            setBackgroundHoverColor(HexToHsb(settings?.button_bg_hover_color));
    
            setTextColor(HexToHsb(settings?.button_text_color));
            setTextHexColor(settings?.button_text_color);
            setTextHoverHexColor(settings?.button_text_hover_color);
            setTextHoverColor(HexToHsb(settings?.button_text_hover_color));
        ...
        {
            const tempSettings = {
                button_text: 'Request Stock',
                button_bg_color: "#121212",
                button_bg_hover_color: "#121212",
                button_text_color: '#ffffff',
                button_text_hover_color: '#ffffff',
                button_border_hex_color: "#ffffff",
                button_border_hover_hex_color: '#ffffff',
                button_border_width: 0,
                button_height: 40,
                button_width: 420,
                button_border_radius: 32,
                button_font_size: 16
            }
            setRequestStockButtonText(tempSettings?.button_text);
            setBackgroundHexColor(tempSettings?.button_bg_color);
            setBackgroundColor(HexToHsb(tempSettings?.button_bg_color));
            setBackgroundHoverHexColor(tempSettings?.button_bg_hover_color);
            setBackgroundHoverColor(HexToHsb(tempSettings?.button_bg_hover_color));
    
            setTextColor(HexToHsb(tempSettings?.button_text_color));
            setTextHexColor(tempSettings?.button_text_color);
            setTextHoverHexColor(tempSettings?.button_text_hover_color);
            setTextHoverColor(HexToHsb(tempSettings?.button_text_hover_color));
    
            setBorderColor(HexToHsb(tempSettings?.button_border_hex_color));
            setBorderHexColor(tempSettings?.button_border_hex_color);
            setBorderHoverHexColor(tempSettings?.button_border_hover_hex_color);
            setBorderHoverColor(HexToHsb(tempSettings?.button_border_hover_hex_color));
    
            setBorderWidth(tempSettings?.button_border_width);
            setButtonHeight(tempSettings?.button_height);
            setButtonWidth(tempSettings?.button_width);
            setButtonRadiusValue(tempSettings?.button_border_radius);
            setButtonFontSizeValue(tempSettings?.button_font_size);
        }
    }

    const isDataChanged = useCallback(() => {
        if(inheritFromThemeRedux !== isInheritFromTheme) return true;
        if (buttonSettingsRedux?.button_text !== requestStockButtonText) return true;        
        if (buttonSettingsRedux?.button_bg_hover_color !== backgroundHoverHexColor) return true;
        if (buttonSettingsRedux?.button_border_hex_color !== borderHexColor) return true;
        if (buttonSettingsRedux?.button_border_hover_hex_color !== borderHoverHexColor) return true;
        if (buttonSettingsRedux?.button_border_radius != buttonRadiusValue) return true;
        if (buttonSettingsRedux?.button_border_width != borderWidth) return true;
        if (buttonSettingsRedux?.button_font_size != buttonFontSizeValue) return true;
        if (buttonSettingsRedux?.button_height != buttonHeight) return true;
        if (buttonSettingsRedux?.button_text_color !== textHexColor) return true;
        if (buttonSettingsRedux?.button_text_hover_color !== textHoverHexColor) return true;
        if (buttonSettingsRedux?.button_width != buttonWidth) return true;
    ...
    const getRequestStockButtonSettings = async () => {
        ...
        if (response.ok) {
            ...
            dispatch(setButtonSettings(JSON.parse(requestStockButtonSettings?.button)));
            dispatch(setButtonInheritFromTheme(requestStockButtonSettings?.button_inherit_from_theme));
            
            setIsInheritFromTheme(requestStockButtonSettings?.button_inherit_from_theme);
            setRequestStockButtonSettings(JSON.parse(requestStockButtonSettings?.button));
    ...
    const saveRequestStockButtonNTextColor = async () => {
        setIsLoading(true);
        
        let buttonSettings = makeButtonSettings();
        const formData = new FormData();
        
        formData.append("button_inherit_from_theme", isInheritFromTheme);
        formData.append("button", buttonSettings);
    ...
    useEffect(() => {
        ...
        else if((inheritFromThemeRedux === 1) && Object.keys(buttonSettingsRedux).length === 0)
            getRequestStockButtonSettings();

        else 
        {
            setRequestStockButtonSettings(buttonSettingsRedux);
            ...
            {isLoading === false && <div className="pt-3">
                ...
                <div className="mt-5">
                    <Button
                        ...
                        onClick={() => saveRequestStockButtonNTextColor()}
                    >
                        Save
                    </Button>
                </div>
