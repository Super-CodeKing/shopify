...
import { setBadgeDesign } from "../../store/reducers/RequestStock";
...
export default function BadgeDesign() {
    ...
    const displayBadgeRedux = useSelector((state) => state.requeststock.badgeDesign);

    const [loading, setLoading] = useState(false);
    const [toastProps, setToastProps] = useState(emptyToastProps);
    const [ribbonBadgePosition, setRibbonBadgePosition] = useState("top-right");
    const [ribbonBadgeText, setRibbonBadgeText] = useState("Request Stock");
    ...
    const [badgeFontSizeValue, setBadgeFontSizeValue] = useState(12);
    ...
    const saveRequestStockBadgeDesign = async () => {
        setLoading(true)
        ...
        const response = await fetch("/api/request-stock/badge-design", {
            method: "POST",
        ...
        if (response.ok) {
            setToastProps({
                content: "Badge Design Saved Successfully!"
            });
            getRequestStockBadgeDesign();
            setLoading(false)
        }
    }

    const setRequestStockBadgeDesign = (requestStockBadgeDesign) => {
        if(Object.keys(requestStockBadgeDesign).length !== 0) {
            setRibbonBadgeText(requestStockBadgeDesign.text);
            setRibbonBadgePosition(requestStockBadgeDesign.position);
            setBackgroundHexColor(requestStockBadgeDesign.bg_color);
            setBadgeTextHexColor(requestStockBadgeDesign.text_color);
            setBadgeFontSizeValue(requestStockBadgeDesign.font_size);
        }
    }

    const isDataChanged = useCallback(() => {
        if(displayBadgeRedux.text !== ribbonBadgeText) return true;
        if(displayBadgeRedux.position !== ribbonBadgePosition) return true;
        if(displayBadgeRedux.bg_color !== backgroundHexColor) return true;
        if(displayBadgeRedux.text_color !== badgeTextHexColor) return true;
        if(displayBadgeRedux.font_size !== badgeFontSizeValue) return true;

        return false;
    }, [ribbonBadgeText, ribbonBadgePosition, backgroundHexColor, badgeTextHexColor, badgeFontSizeValue, displayBadgeRedux]);

    const getRequestStockBadgeDesign = async () => {
        const response = await fetch("/api/request-stock/badge-design");
        if (response.ok) {
            const requestStockBadgeDesign = await response.json();
            setRequestStockBadgeDesign(requestStockBadgeDesign);
            dispatch(setBadgeDesign(requestStockBadgeDesign));
            setLoading(false);
        } else {
            setLoading(false);
            throw new Error(`HTTP error ${response.status}`);
    ...
    useEffect(() => {
        setLoading(true);
        if(Object.keys(displayBadgeRedux).length === 0) getRequestStockBadgeDesign();
        else {
            console.log(displayBadgeRedux);
            setRequestStockBadgeDesign(displayBadgeRedux);
            setLoading(false);
        };
    }, []);
                ...
                {loading === false && <Page fullWidth>
                                ...
                                <div className="flex">
                                    <div className="grow">
                                        <RadioButton
                                            ...
                                            id="requestStockRibbonTopLeft"
                                            name="requestStockRibbonBadgePosition"
                                            onChange={(event) =>
                                ...
                                <div className="flex mb-3">
                                    <div className="grow">
                                        <RadioButton
                                            ...
                                            id="requestStockRibbonBottomLeft"
                                            name="requestStockRibbonBadgePosition"
                                            onChange={(event) =>
                                                changeRibbonBadgePosition(
                                                    event,
                                    ...
                                    <div className="grow">
                                        <RadioButton
                                            label="Bottom Right"
                                            id="requestStockRibbonBottomRight"
                                            name="requestStockRibbonBadgePosition"
                                            checked={
                    ...
                    <div className="mt-3">
                        <Button
                            ...
                            onClick={() => saveRequestStockBadgeDesign()}
                        >
                            Save
                        </Button>
                    </div>
