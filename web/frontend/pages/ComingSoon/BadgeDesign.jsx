...
import { setBadgeDesign } from "../../store/reducers/ComingSoon";
...
export default function BadgeDesign() {
    ...
    const displayBadgeRedux = useSelector((state) => state.comingsoon.badgeDesign);

    const [loading, setLoading] = useState(false);
    const [toastProps, setToastProps] = useState(emptyToastProps);
    const [ribbonBadgePosition, setRibbonBadgePosition] = useState("top-right");
    const [ribbonBadgeText, setRibbonBadgeText] = useState("Coming Soon");
    ...
    const saveComingSoonBadgeDesign = async () => {
        setLoading(true)
        ...
        const response = await fetch("/api/coming-soon/badge-design", {
            method: "POST",
        ...
        if (response.ok) {
            setToastProps({
                content: "Coming Soon Badge Design Saved Successfully!"
            });
            getComingSoonBadgeDesign();
            setLoading(false)
        }
    }

    const setComingSoonBadgeDesign = (comingSoonBadgeDesign) => {
        if(Object.keys(comingSoonBadgeDesign).length !== 0) {
            setRibbonBadgeText(comingSoonBadgeDesign.text);
            setRibbonBadgePosition(comingSoonBadgeDesign.position);
            setBackgroundHexColor(comingSoonBadgeDesign.bg_color);
            setBadgeTextHexColor(comingSoonBadgeDesign.text_color);
            setBadgeFontSizeValue(comingSoonBadgeDesign.font_size);
        }
    }
    ...
    const getComingSoonBadgeDesign = async () => {
        const response = await fetch("/api/coming-soon/badge-design");
        if (response.ok) {
            const comingSoonBadgeDesign = await response.json();
            setComingSoonBadgeDesign(comingSoonBadgeDesign);
            dispatch(setBadgeDesign(comingSoonBadgeDesign));
            setLoading(false);
        } else {
            setLoading(false);
            throw new Error(`HTTP error ${response.status}`);
    ...
    useEffect(() => {
        setLoading(true);
        if(Object.keys(displayBadgeRedux).length === 0) getComingSoonBadgeDesign();
        else {
            console.log(displayBadgeRedux);
            setComingSoonBadgeDesign(displayBadgeRedux);
            setLoading(false);
        };
    }, []);
                ...
                {loading === false && <Page fullWidth>
                    ...
                    <div className="mt-3">
                        <Button
                            ...
                            onClick={() => saveComingSoonBadgeDesign()}
                        >
                            Save
                        </Button>
                    </div>
